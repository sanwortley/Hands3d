...
const ProductsSection: React.FC<ProductsSectionProps> = ({ 
  ...
  const activeModel = models[currentIdx] || models[0];

  const handlePrev = () => {
    const prevIdx = (currentIdx - 1 + models.length) % models.length;
    setSelectedModel(models[prevIdx].url);
  };

  const handleNext = () => {
    const nextIdx = (currentIdx + 1) % models.length;
    setSelectedModel(models[nextIdx].url);
  };
    ...
    >
      ...
      <div className="w-full max-w-7xl mx-auto px-6 md:px-12 flex-1 flex flex-col justify-between pt-24 pb-4 md:pt-28 md:pb-6 relative z-20 pointer-events-none">
        ...
        <button 
          ...
          title={lang === 'es' ? 'Modelo anterior' : 'Previous model'}
        >
        ...
        <div className="w-full flex flex-col md:flex-row justify-between items-end mt-auto pb-4 gap-4">
          ...
          {/* Active Product Name at the Bottom-Left */}
          ...
          <div className="flex justify-center items-center gap-2 select-none pointer-events-auto mx-auto md:mx-0">
            {models.map((m, idx) => {
              const isActive = m.url === selectedModel;
              return (
                <button
                  key={m.url}
      ...
      <div 
        className={`absolute right-0 top-[20%] h-[68%] max-h-[480px] z-45 flex items-center transition-transform duration-500 ease-in-out ${
          isFichaOpen ? 'translate-x-0' : 'translate-x-[320px] md:translate-x-[380px]'
        }`}
      >
        ...
        >
          ...
          <div className="text-[#3E5F8A] mt-1">
            <svg 
              className={`w-5 h-5 transition-transform duration-500 ${isFichaOpen ? 'rotate-180' : ''}`} 
              fill="none" 
              stroke="currentColor" 
              strokeWidth="2.5" 
              viewBox="0 0 24 24"
        ...
        >
          ...
          <div className="flex flex-col items-center text-center">
            <span className="font-space text-[10px] text-[#B58E45] uppercase tracking-widest font-bold mb-1 select-none">
              {lang === 'es' ? 'Modelado & Fabricación' : 'Design & Production'}
            </span>
          ...
          <div className="flex-1 flex flex-col justify-center my-3">
            <div className="grid grid-cols-1 gap-2.5 text-[11px] md:text-[12px] font-outfit">
              <div className="flex justify-between border-b border-[#FAF5EF]/15 pb-1">
                <span className="opacity-75">{lang === 'es' ? 'Material:' : 'Material:'}</span>
                <span className="font-bold text-[#B58E45]">{activeModel.specs.material}</span>
              </div>
              <div className="flex justify-between border-b border-[#FAF5EF]/15 pb-1">
                <span className="opacity-75">{lang === 'es' ? 'Resolución:' : 'Resolution:'}</span>
                <span className="font-bold">{activeModel.specs.resolution}</span>
              </div>
              <div className="flex justify-between border-b border-[#FAF5EF]/15 pb-1">
                <span className="opacity-75">{lang === 'es' ? 'Relleno:' : 'Infill:'}</span>
                <span className="font-bold">{activeModel.specs.infill}</span>
              </div>
              <div className="flex justify-between border-b border-[#FAF5EF]/15 pb-1">
                <span className="opacity-75">{lang === 'es' ? 'Tiempo de Impresión:' : 'Printing Time:'}</span>
                <span className="font-bold">{activeModel.specs.time}</span>
              </div>
              <div className="flex justify-between border-b border-[#FAF5EF]/15 pb-1">
                <span className="opacity-75">{lang === 'es' ? 'Dimensiones:' : 'Dimensions:'}</span>
                <span className="font-bold text-[10px] md:text-[11px] truncate max-w-[155px]" title={activeModel.specs.dimensions}>
                  {activeModel.specs.dimensions}
                </span>
              </div>
              <div className="flex justify-between border-b border-[#FAF5EF]/15 pb-1">
                <span className="opacity-75">{lang === 'es' ? 'Peso:' : 'Weight:'}</span>
                <span className="font-bold">{activeModel.specs.weight}</span>
              </div>
              <div className="flex justify-between pt-0.5">
                <span className="opacity-75">{lang === 'es' ? 'Resistencia Mecánica:' : 'Mechanical Strength:'}</span>
                <span className="font-bold text-[#B58E45]">{activeModel.specs.strength}</span>
              </div>
            </div>
          </div>
          ...
          <div className="text-[10px] md:text-[11px] text-center font-outfit opacity-80 leading-relaxed border-t border-[#FAF5EF]/15 pt-2 px-1 select-text">
            {lang === 'es' 
              ? 'Piezas funcionales optimizadas para máxima resistencia y detalle superficial.' 
              : 'Functional components optimized for durability and fine resolution.'}
          </div>
        </div>
      </div>
